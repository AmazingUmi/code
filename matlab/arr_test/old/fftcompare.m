...
for k = 1:length(sig_freq)
    mid_signal = sig_amplitude(k)*cos(2*pi*sig_freq(k)*t+sig_phase(k));
    recover_sig_n = recover_sig_n + mid_signal;
end
toc
subplot(3,1,1)
plot(t,signal)
ylim([-1,1]);
subplot(3,1,2)
plot(t,recover_sig_n)
ylim([-1,1]);
subplot(3,1,3)
plot(t,ifft_signal)
ylim([-1,1]);
figure
dsig = signal - recover_sig_n;
plot(dsig)
...
% 
% % 1. 计算均方误差 (MSE)
MSE = mean((signal - recover_sig_n).^2);
% 
% % 2. 计算信噪比 (SNR)
signal_power = sum(signal.^2);   % 原始信号的功率
noise_power = sum((signal - recover_sig_n).^2);  % 噪声的功率
SNR = 10 * log10(signal_power / noise_power);
% 
% % % 3. 计算结构相似性指数 (SSIM)
[ssim_value, ssim_map] = ssim(recover_sig_n, signal);  % 计算 SSIM 和局部 SSIM 图
fprintf('SSIM: %.4f\n', ssim_value);
