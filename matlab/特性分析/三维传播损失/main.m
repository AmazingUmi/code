...
parfor j=1
    ...
    centerpp = [];
    if j<6
        centerpp(j).lon=Centerp.lon-0.5+0.25*(j-1);
        centerpp(j).lat=Centerp.lat-0.5;
    elseif j>5 && j<11
        centerpp(j).lon=Centerp.lon-0.5+0.25*(j-6);
        centerpp(j).lat=Centerp.lat-0.25;
    elseif j>10 && j<16
        centerpp(j).lon=Centerp.lon-0.5+0.25*(j-11);
        centerpp(j).lat=Centerp.lat;
    elseif j>15 && j<21
        centerpp(j).lon=Centerp.lon-0.5+0.25*(j-16);
        centerpp(j).lat=Centerp.lat+0.25;
    else j>20
        centerpp(j).lon=Centerp.lon-0.5+0.25*(j-21);
        centerpp(j).lat=Centerp.lat+0.5;
    end
    %文件编号
    n=num2str(j);
    envfil= ['pos',n];
    % 声速板块
    [~, ssp_raw, ~] = get_env(ETOPO,WOA18,centerpp(j).lat,centerpp(j).lon, MouthIdx); % 获取平均声速剖面
    ...
    for i=1:Ndir
        azi=360/Ndir*(i-1);
        targetp = [];
        [centerpp(j), targetp, rmax, azi] = coord_proc(centerpp(j), targetp, rmax, azi);
        
        % 环境 env、bty、ssp
        call_Bellhop_surface_more(ETOPO,WOA18,envfil, freq, SourthDepth,ReceiveDepth, MouthIdx, centerpp(j), targetp, ...
...
for j=1:8
    ...
    TLT(j,Ndir+1,:) = TLT(j,1,:);
    polarPcolor(R,tht,squeeze(TLT(j,:,:)),'Nspokes',8)
    clim([40 130])
    shading interp
    tej = flipud( jet( 256 ) );
    colormap( tej )
