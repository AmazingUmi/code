...
#include <cstdlib>
#include <chrono>
#include <unistd.h>  // macOS/Linux 下用于 chdir 函数
///usr/bin/clang++ -Xpreprocessor -fopenmp -I/Users/luyiyang/miniconda3/include -L/Users/luyiyang/miniconda3/lib -lomp -Wl,-rpath,/Users/luyiyang/miniconda3/lib BellParallelMac.cpp -o BellParallelMac
// 改变当前工作目录
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        ...
        std::cerr << "示例: ./BellParallelMac /path/to/env/directory" << std::endl;
        return 1;
    }
    ...
    // 获取当前目录（假设 bellhop.exe 在当前目录下）
    char cwd[1024];
    if (getcwd(cwd, sizeof(cwd)) == NULL) {
        std::cerr << "无法获取当前工作目录" << std::endl;
        return 1;
    }
    std::string bellhopPath = std::string(cwd) + "/bellhop.exe";
