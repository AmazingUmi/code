...
### Windows 批量并行工具（`C++code/for_win/`）

- `parallel.cpp` - Windows 下批量/并行调用 Bellhop 的命令行工具（OpenMP）
  - **调用方式**：在命令行运行 `parallel "target path"`
  - **说明**：
    - `target path` 是包含 `env_files_list.txt` 的目录（程序会先切换到该目录再读取列表）
    - `bellhop.exe` 需要与 `parallel.exe` 放在**同一目录**，程序会自动定位并调用它（不依赖当前工作目录）
...
当前 A5 的批量合成在底层使用 `function/TargSigGenerator.m`，其接口已扩展支持 GPU：

- `TargSigGenerator(AnalyRecord, AnalyFreq, ArrRDSig, Amp_source, fs, Ndelay, TargSegNum, useGPU, BatchSize)`
  - **`useGPU`**：`true/false`，为空时自动检测 `gpuDeviceCount`
  - **`BatchSize`**：GPU 批处理大小；`>1` 时启用频率聚合模式（通常可提升 GPU 利用率）

#### 显存占用看似“不释放”的说明

MATLAB GPU 计算通常会对显存进行**池化/缓存复用**，因此在长循环批处理（大量信号×深度×环境）时，显存曲线可能表现为持续升高或不回落，但这不一定是泄漏。

项目中 `function/SignalPatchGenerator.m` 已做了以下处理以改善长时间运行时的显存压力：

- 自动检测 GPU 是否可用（避免无 GPU 时强制走 GPU）
- 每次合成后 `wait(gpuDevice)`，帮助释放临时内存回到池中
- 循环内及时清空大变量引用（例如 `TargSig/TargT`）
- 预留周期性 `reset(gpuDevice)` 开关（仅在确实出现 OOM 时建议启用，会牺牲一定性能）
...
- **`ArrReader.m`**：对批量 Bellhop `.arr` 文件进行读取、排序和幅值门限过滤，生成压缩版到达结构 `ENV_ARR_less.mat`，是 A4 步骤的核心实现之一。
- **`SignalPatchGenerator.m` + `TargSigGenerator.m`**：在给定到达结构和源信号频谱的情况下，批量合成不同深度、不同源文件的接收信号，是 A5 步骤的具体实现。
- **`FreqDuplicator.m`**：针对单个基础 `.env` 文件，按频率向量自动生成多组 `test_i.env/.bty/.ssp/.trc/.brc` 文件，并输出 `env_files_list.txt`，可作为 `A3ENVDuplicator.m` 的底层工具或独立使用。
- **`FileMakerIdeal.m`**：当地形 (`BTY`) 和声速剖面 (`SSPIN`) 已经在外部准备好时，用于直接生成一套可用于 Bellhop 的理想化环境文件（`.env/.bty/.ssp/.trc/.brc`）。
- **可视化工具**：
  - `plotSSP.m`：二维声速剖面可视化（Range–Depth–c）。
  - `plotshd.m`：Bellhop `.shd` 声场(TL) 可视化，支持按频率和子图布局选择。
  - `plotray.m`：Bellhop 射线 `.ray` 文件可视化，区分直达、海面反射、海底反射、多次反射路径。

这些脚本不改变主流程 A1–A5 的大结构，但为调试、可视化和自定义实验提供了更细粒度的接口。

### 接口文档

项目提供了单独的接口说明文件：

- **`doc\接口文档.md`**：对 `function/` 目录中主要函数（如 `load_data`, `get_env`, `ReCoeTop`, `RefCoeBw`, `wavfreq`, `SigGenerateTD`, `ArrReader`, `SignalPatchGenerator` 等）的**输入输出、使用示例和注意事项**做了系统整理。

当需要在主流程之外复用这些基础能力（例如仅做环境提取或仅做信号合成实验）时，建议结合本说明文档和 `接口文档.md` 一起阅读。
