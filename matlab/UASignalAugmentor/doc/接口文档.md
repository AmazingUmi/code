...
### 7.2 read_arrivals_asc

**功能**：读取 Bellhop/Bellhop3D 生成的 ASCII 格式 `.arr` 到达结构文件
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| `ARRFile` | char | ASCII 格式到达结构文件名（含或不含扩展名） |

**输出参数**：
| 参数名 | 类型 | 说明 |
|--------|------|------|
| `Arr` | struct[...] | 多维结构体数组，按接收距离/深度/源深度等组织的到达信息 |
| `Arr(i).Narr` | int16 | 该接收点的声线条数 |
| `Arr(i).A` | single[] | 复振幅 \(A = amp \cdot e^{j\phi}\) |
| `Arr(i).delay` | single[] | 复时延（实部为传播时延，虚部为扩散项） |
| `Arr(i).SrcDeclAngle` | single[] | 源仰角 (deg) |
| `Arr(i).RcvrDeclAngle` | single[] | 接收仰角 (deg) |
| `Arr(i).NumTopBnc` | int16[] | 海面反射次数 |
| `Arr(i).NumBotBnc` | int16[] | 海底反射次数 |
| `Pos` | struct | 源/接收几何信息，含 `.freq`、`Pos.s.*`、`Pos.r.*` 等字段 |

**典型用途**：
- 读取 Bellhop 计算结果，对到达结构进行自定义后处理或可视化
- 为 `ArrReader`、`A4ArrProcessor` 等高层接口提供低层读入能力

---

### 7.3 read_arrivals_bin

**功能**：读取 Bellhop/Bellhop3D 生成的 BINARY（二进制）格式 `.arr` 到达结构文件
...
**输入/输出参数**：
- 与 `read_arrivals_asc` 类似，只是文件格式为二进制；
  内部通过 Fortran 无格式记录标记 `marker_len` 适配不同编译器的输出。

**注意事项**：
- 若误将 ASCII 文件传给本函数，会抛出 `not a BINARY format Arrivals file?` 错误
- 若读取结果混乱，可根据需要调整源码中的 `marker_len`（一般保持默认值 1 即可）

---

### 7.4 ArrReader

**功能**：批量读取多条 `.arr` 到达结构文件，过滤弱声线并压缩保存为 `ARR` 结构体

**语法**：
```matlab
ArrReader(ArrFileName, EnvSiteRrDir, MAX_FREQ_LIMIT, ...
          AMP_THRESHOLD_RATIO, OutputMatName, OutputDir)
```

**输入参数**：
| 参数名 | 类型 | 说明 |
|--------|------|------|
| `ArrFileName` | cellstr | 不含扩展名的 `.arr` 文件名列表（如 `{'test_1','test_2',...}`） |
| `EnvSiteRrDir` | char | `.arr` 文件所在目录 |
| `MAX_FREQ_LIMIT` | double | 最大处理频率，超过该值的文件会被跳过 |
| `AMP_THRESHOLD_RATIO` | double | 振幅门限比例，保留幅值 ≥ 此比例 × 最大幅值 的声线 |
| `OutputMatName` | char | (可选) 输出 `.mat` 文件名，默认 `'ENV_ARR_less.mat'` |
| `OutputDir` | char | (可选) 输出目录，默认为 `EnvSiteRrDir` 的上一级目录 |

**输出**：
- 在 `OutputDir` 中生成 `OutputMatName`，包含结构体数组：
  - `ARR(m, n).Amp`：第 `m` 个文件、第 `n` 个接收深度的声线幅值
  - `ARR(m, n).Delay`：对应时延
  - `ARR(m, n).phase`：相位
  - `ARR(m, n).freq`：频率
  - `ARR(m, n).rd`：接收深度
...
**典型用途**：
- 实现 A4 步骤中对大量 `.arr` 文件的批量压缩与阈值过滤
- 为后续的接收信号合成（A5）提供紧凑的到达结构输入

---

### 7.5 SignalPatchGenerator

**功能**：在给定一组到达结构和源信号分析结果的前提下，批量合成多深度、多信号的接收信号

**语法**：
```matlab
SignalPatchGenerator(ARR, SigFilesList, SigFilesStruct, ...
                     OutputSigDir, Analy_freq_all, Amp_source, fs)
```

**输入参数**：
| 参数名 | 类型 | 说明 |
|--------|------|------|
| `ARR` | struct[M×N] | 到达结构数组（通常由 `ArrReader` 生成） |
| `SigFilesList` | struct[] | 原始信号文件信息（如 `dir` 结果） |
| `SigFilesStruct` | struct[] | 对应每个信号的频率分析结果结构体（含 `Analy_freq`、`Analyrecord`、`Ndelay`） |
| `OutputSigDir` | char | 输出接收信号 `.mat` 文件目录 |
| `Analy_freq_all` | double[] | 全局频率集合（A1 生成） |
| `Amp_source` | double | 源强度标量 |
| `fs` | double | 采样率 (Hz) |

**输出效果**：
- 对每个接收深度、每个源信号生成一个目标信号文件：
  - 文件名：`<信号名>_Rd_<depth>_new.mat`
  - 内容：`TargSig`（接收信号），`TargT`（时间轴）

**内部依赖**：
- 调用 `TargSigGenerator` 完成单个信号在指定深度下的合成

---

### 7.6 TargSigGenerator

**功能**：在单一接收深度下，根据到达结构和源信号频谱，合成目标接收信号
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| `AnalyRecord` | struct[] | 信号分段频谱记录（`wavfreq`/`wavfilter` 输出） |
| `AnalyFreq` | double[] | 该信号的频率数组 (Hz) |
| `ArrRDSig` | struct[] | 对应频率的到达结构（同一接收深度） |
| `Amp_source` | double | 源强度标量 |
| `fs` | double | 采样率 (Hz) |
| `Ndelay` | double[] | 分段时间索引（秒） |
| `TargSegNum` | int | (可选) 参与合成的连续段数，默认使用全部可用段 |

**输出参数**：
| 参数名 | 类型 | 说明 |
|--------|------|------|
| `TargSig` | double[] | 合成后的接收时域信号（已取实部并去除尾部零） |
| `TargT` | double[] | 对应时间轴 (s) |

**实现要点**：
- 对每个片段、每个有效频率调用 `SigGenerateTD`，并按绝对时延与片段起始时间对齐后叠加
- 自动根据所有声线的 \(\min/\max\) 时延计算输出长度

---

### 7.7 FileMakerIdeal

**功能**：基于理想（已给定）的地形和声速剖面，生成 Bellhop 所需的完整环境文件集合

**语法**：
```matlab
FileMakerIdeal(BTY, SSPIN, FileName, Config)
```

**输入参数**：
| 参数名 | 类型 | 说明 |
|--------|------|------|
| `BTY` | struct | 海底地形数据，含字段 `.r`, `.d` |
| `SSPIN` | struct | 声速输入结构体，至少包含 `ssp_raw`、`SSProf` 等字段 |
| `FileName` | char | 输出文件名前缀 |
| `Config` | struct | 配置结构体，格式与 `FileMaker` 的 `Config` 基本一致 |

**输出文件**：
- `FileName.env`：环境文件（调用 `write_env` 生成）
- `FileName.bty`：地形文件（调用 `write_bty`）
- `FileName.ssp`：声速剖面文件（按需要调用 `write_ssp`）
- `FileName.trc`：海面反射系数（调用 `ReCoeTop`）
- `FileName.brc`：海底反射系数（调用 `RefCoeBw`）

**适用场景**：
- 已有理想化的地形/声速数据，希望快速构造一套可直接用于 Bellhop 的输入文件

---

### 7.8 FreqDuplicator

**功能**：对单个基础 `.env` 环境文件进行频率扩展，为一组频率生成对应的 `test_i.env/.bty/.ssp/.trc/.brc` 文件，并输出列表

**语法**：
```matlab
NewFileNames = FreqDuplicator(SourceDir, EnvName, OutputDir, ...
                              FreqVector, SeaState, BotType, BotAlpha)
```

**输入参数**：
| 参数名 | 类型 | 说明 |
|--------|------|------|
| `SourceDir` | char | 基础环境文件所在目录 |
| `EnvName` | char | 基础环境文件前缀（不含扩展名），可留空自动检测 |
| `OutputDir` | char | 频率扩展后文件的输出目录 |
| `FreqVector` | double[] | 需要扩展的频率列表 (Hz) |
| `SeaState` | int | (可选) 海况等级，用于重新计算 `.trc` |
| `BotType` | char | (可选) 海底类型（传入 `RefCoeBw` 的 `base_type`） |
| `BotAlpha` | double | (可选) 底质吸收系数 (dB/λ) |

**输出参数**：
| 参数名 | 类型 | 说明 |
|--------|------|------|
| `NewFileNames` | cellstr | 新生成环境文件前缀，如 `{'test_1', 'test_2', ...}` |

**附加输出**：
- 在 `OutputDir` 下生成 `env_files_list.txt`，列出所有新文件名前缀（用于 A3/A4 批处理）

**实现要点**：
- 在基础 `.env` 文本中定位频率行和海底参数行，按频率重写并自适应调整吸收系数
- 自动复制 `.bty/.ssp/.sbp` 等关联文件，并按需重新计算 `.trc/.brc`
...
**主要参数**：
- `SSP`：包含字段 `.c` 的结构体，大小为 `[Nz × Nr]`
- `r`：距离向量 (km)
- `z`：深度向量 (m)
- `title_str`：可选标题字符串

**说明**：
- 自动检查并在必要时转置 `SSP.c` 以匹配 `r/z` 维度
- 使用 `imagesc + contour` 绘制伪彩色声速场并叠加等值线

---

#### 7.9.2 plotshd

**功能**：绘制 Bellhop 输出的 `.shd` 声场（Transmission Loss, TL）

**语法**（常用形式）：
```matlab
plotshd(filename)
plotshd(filename, freq)
plotshd(filename, m, n, p)
plotshd(filename, freq, m, n, p)
```

**说明**：
- 默认使用 `.shd` 扩展名，并选取第一个频率、方位和源深度
- 支持按频率筛选以及子图布局 (subplot)
- 内部调用 `read_shd` 读取声场数据，并将复压强转换为 TL(dB) 后用 `pcolor` 绘制

---

#### 7.9.3 plotray

**功能**：绘制 Bellhop/Bellhop3D 的射线文件 `.ray`
...
**说明**：
- `rayfil` 为射线文件前缀或完整文件名（可省略 `.ray` 扩展名）
- 支持 2D (`'rz'`) 与 3D (`'xyz'`) 射线输出，3D 情况下自动转换为 \(r\text{-}z\) 坐标
- 按是否触及海面/海底用不同颜色区分射线路径
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| `clim` | double[1×2] | 颜色轴上下限 `[cmin, cmax]` |

**用途**：
- 常与 `plotshd` 等 TL 图一起使用，使得较小的 TL（能量强）显示在颜色条顶部
