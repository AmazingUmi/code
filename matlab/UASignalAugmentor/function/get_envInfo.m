function [Temp, Sal, Depth] = get_envInfo(WOA, lon, lat, timeIdx)
%% 提取给定坐标和时间的温盐深数据。
...
if timeIdx <= 12     
    ...
    TEMP = WOA.Data{17}.Temp;
    SAL = WOA.Data{17}.Sal;
    Depth = WOA.Data{17}.Depth;
    Nd = length(WOA.Data{timeIdx}.Depth);     % 第timeIdx月声速剖面数据的深度个数
    TEMP(:,:,1:Nd) = WOA.Data{timeIdx}.Temp(:,:,:);
    SAL(:,:,1:Nd) = WOA.Data{timeIdx}.Sal(:,:,:);
else
    TEMP = WOA.Data{timeIdx}.Temp;
    SAL = WOA.Data{timeIdx}.Sal;
    Depth = WOA.Data{timeIdx}.Depth;
end
