function FileMakerIdeal(BTY, SSPIN, FileName, Config)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FILEMAKER 生成Bellhop声场计算所需的环境文件
%   理想环境数据和配置参数，生成Bellhop计算所需的各类文件
%
% 输入参数:
%   BTY - 海底地形数据结构体
%       .r, .d - 距离和深度数据
...
bathm   = BTY;            % 海底地形数据
ssp_raw = SSPIN.ssp_raw;  % 声速剖面原始数据
SSProf  = SSPIN.SSProf;   % 声速剖面矩阵数据
...
Beam.Box.r = ceil(1.5*rmax + 1);    % 计算最大距离（需大于最大接收距离）
...
% 生成海底地形文件 (*.bty)
if  length(Config.Cal.Bdry.Bot.Opt) >=2 && Config.Cal.Bdry.Bot.Opt(2) == '*'
    write_bty(FileName, "'LS'", bathm);
end

% 生成声速剖面文件 (*.ssp)
if Config.Cal.Bdry.Top.Opt(1) == 'Q'
